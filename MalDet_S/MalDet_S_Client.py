...
headers = {'IP': get_ip(), 'MFL': 'False', 'date': '', 'search_ip': '', 'cookie': ''}
...
def send_many_files(address: str, path: str, mode=1):
    ...
    for file in files:
        pprint(send_file_to_analysis(file, mode=MODE))
    response = requests.get(f'{address}/files/analysis/{mode}',
                            headers=headers)
    return json.loads(response.text)


def search_file(address: str, h='123321', mode='hash', ip='', date=''):
    if mode == 'ip':
...
print("[+] Time of working is: "+str(t2-t1))
